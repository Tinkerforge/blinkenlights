...
def freeze_images():
    directory = 'starter_kit_blinkenlights_demo'
    image_files = []
...
def build_macosx_pkg():
    ...
    system('cd starter_kit_blinkenlights_demo; python build_ui.py')
    ...
    resources_path = os.path.join(dist_path, 'Starter Kit Blinkenlights Demo.app', 'Contents', 'Resources')
    ...
    dmg_name = 'starter_kit_blinkenlights_demo_macos_{0}.dmg'.format(DEMO_VERSION.replace('.', '_'))
...
def build_windows_pkg():
    ...
    #    sign_py2exe('dist\\starter_kit_blinkenlights_demo.exe')

    print('creating NSIS script from template')
    nsis_template_path = os.path.join(root_path, 'build_data', 'windows', 'nsis', 'starter_kit_blinkenlights_demo_installer.nsi.template')
    nsis_path = os.path.join(dist_path, 'nsis', 'starter_kit_blinkenlights_demo_installer.nsi')
    ...
    system('"C:\\Program Files\\NSIS\\makensis.exe" dist\\nsis\\starter_kit_blinkenlights_demo_installer.nsi')
    installer = 'starter_kit_blinkenlights_demo_windows_{0}.exe'.format(DEMO_VERSION.replace('.', '_'))
...
def build_linux_pkg():
    ...
    print('removing old build directories')
    dist_path = os.path.join(root_path, 'dist')
    egg_info_path = os.path.join(root_path, 'starter_kit_blinkenlights_demo.egg-info')
    ...
    print('copying build data')
    build_data_path = os.path.join(root_path, 'build_data', 'linux', 'starter_kit_blinkenlights_demo')
    linux_path = os.path.join(dist_path, 'linux')
    shutil.copytree(build_data_path, linux_path)

    print('unpacking sdist tar file')
    system('tar -x -C {0} -f {0}/starter_kit_blinkenlights_demo-{1}.tar.gz starter_kit_blinkenlights_demo-{1}/starter_kit_blinkenlights_demo'.format(dist_path, DEMO_VERSION))

    print('copying unpacked demo source')
    unpacked_path = os.path.join(dist_path, 'starter_kit_blinkenlights_demo-{0}'.format(DEMO_VERSION), 'starter_kit_blinkenlights_demo')
    linux_share_path = os.path.join(linux_path, 'usr', 'share', 'starter_kit_blinkenlights_demo')
    ...
    system('dpkg -b dist/linux starter-kit-blinkenlights-demo-{0}_all.deb'.format(DEMO_VERSION))

    print('changing owner back to original user')
    system('sudo chown -R `logname`:`logname` dist/linux')

    #print('checking Debian package')
    #system('lintian --pedantic starter-kit-blinkenlights-demo-{0}_all.deb'.format(DEMO_VERSION))
