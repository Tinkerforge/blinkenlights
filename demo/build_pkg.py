...
UNDERSCORE_NAME = 'starter_kit_blinkenlights_demo'
CAMEL_CASE_NAME = 'Starter Kit Blinkenlights Demo'

def build_linux_pkg():
    print('building demo Debian package')
    utils = BuildPkgUtils(UNDERSCORE_NAME, 'linux', DEMO_VERSION, '--internal' in sys.argv)

    utils.run_sdist(prepare_script=os.path.join(utils.root_path, 'build_ui.py'))
...
def build_pyinstaller_pkg():
    ...
    utils.build_pyinstaller_pkg(prepare_script=os.path.join(utils.root_path, 'build_ui.py'))
